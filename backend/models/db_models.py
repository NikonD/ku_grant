...
class Specialty(Base):
    ...
    item_comb   = Column(String(100), nullable=False) 
    description = Column(Text)
    price_paid  = Column(Integer)
    created_at  = Column(DateTime(timezone=True), server_default=func.now())

    # Связь с проходными баллами (один ко многим)
...
class PassingScore(Base):
    ...
    quota        = Column(String(50), nullable=False)   # general, rural, orphan
    applicants   = Column(Integer, default=0)
    enrolled     = Column(Integer, default=0)

    # Обратная связь со специальностью
